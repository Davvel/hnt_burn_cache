#!/usr/bin/env python3
"""Offline smoke tests for v1.3.0 helpers. No Dune credits are used."""
...
os.environ.setdefault("CACHE_VERSION", "1.3.0")
...
# On 2026-09-02 the cache must end on 2026-08-31 (T-2), excluding Sep 1 and Sep 2.
today = date(2026, 9, 2)
rows = []
for i in range(32):
...
assert first_day.isoformat() == "2026-08-02", first_day
assert len(daily) == 30, len(daily)
assert daily[0]["date"] == "2026-08-02", daily[0]
assert daily[-1]["date"] == "2026-08-31", daily[-1]
assert all(r["date"] not in {"2026-09-01", "2026-09-02"} for r in daily)
assert skipped == 3, skipped  # today + yesterday + old row
assert date_col == "day"
assert value_col == "hnt_burned"

print("v1.3.0 offline smoke tests passed: 30 settled days ending at T-2.")
