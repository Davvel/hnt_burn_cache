#!/usr/bin/env python3
"""Offline smoke tests for v1.5.0 helpers. No Dune credits are used."""
...
os.environ.setdefault("CACHE_VERSION", "1.5.0")
...
# On 2026-09-04 the cache includes Sep 4 (T), while the app should display
# only through Sep 2 (T-2). 32 cache rows give the app 30 display days.
today = date(2026, 9, 4)
rows = []
for i in range(32):
...
assert len(daily) == 32, len(daily)
assert daily[0]["date"] == "2026-08-04", daily[0]
assert daily[-1]["date"] == "2026-09-04", daily[-1]
assert any(r["date"] == "2026-09-04" for r in daily)
assert skipped == 1, skipped  # old row only
assert date_col == "day"
...
print("v1.5.0 offline smoke tests passed: cache through TODAY; 30 display rows through T-2.")
